...
const CreatePatientModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
            {/* Modal Container: Max-height ensures it doesn't cut off on small phones */}
            <div className="bg-white w-full max-w-lg rounded-[24px] sm:rounded-3xl shadow-2xl border border-slate-100 flex flex-col max-h-[95vh] animate-in zoom-in-95 duration-200">
                
                {/* Header: Fixed */}
                <div className="px-6 py-4 border-b border-slate-50 flex items-center justify-between bg-gradient-to-r from-sky-50 to-indigo-50 shrink-0">
                    <h2 className="text-lg sm:text-xl font-black text-slate-800 flex items-center gap-3">
                ...
                {/* Body: Scrollable area for mobile */}
                <div className="overflow-y-auto p-6 custom-scrollbar">
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        {/* Footer: Now inside the scrollable area or can be moved outside if preferred */}
                        <div className="flex flex-col-reverse sm:flex-row gap-3 pt-4">
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full sm:flex-1 px-8 py-3 bg-gradient-to-r from-sky-600 to-indigo-600 text-white rounded-2xl font-bold text-sm shadow-lg shadow-sky-200 hover:shadow-xl hover:scale-[1.02] transition-all active:scale-95 disabled:opacity-50 disabled:pointer-events-none flex items-center justify-center gap-2"
                            >
                                {loading ? (
                                    <>
