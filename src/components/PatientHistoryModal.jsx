...
const PatientHistoryModal = ({ isOpen, onClose, patient }) => {
    ...
    useEffect(() => {
        if (isOpen && patient?.id) {
            ...
            setAppointments([]); // Reset when closed or no patient
        }
    }, [isOpen, patient]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-100 flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <div className="overflow-y-auto p-6 space-y-8">
                    {/* Patient Info Card */}
                    <div className="bg-linear-to-br from-indigo-50 to-sky-50 rounded-2xl p-6 border border-indigo-100/50 relative overflow-hidden">
                        ...
                        <div className="flex items-start gap-5 relative z-10">
                            <div className="w-16 h-16 rounded-full bg-white shadow-sm flex items-center justify-center text-indigo-600 font-bold text-xl border-4 border-white/50">
                                {patient.initials || (patient.firstName?.[0] + patient.lastName?.[0]) || <User className="w-8 h-8" />}
                            </div>
                            <div className="flex-1">
                                <h3 className="text-2xl font-black text-slate-900 mb-1">{patient.name || `${patient.firstName} ${patient.lastName}`}</h3>
                                        ...
                                        <div className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                                                <div className="font-bold text-slate-900">
                                                    ...
                                                    <span className="ml-2 text-xs font-normal text-slate-500 flex-inline items-center gap-1">
                                                    </span>
                                                </div>
                                                <span className={`px-2.5 py-1 rounded-full text-xs font-bold w-fit ${getStatusColor(apt.status)}`}>
                                                    {apt.status}
                                                </span>
                                            </div>

                                            <div className="flex items-center gap-2 text-sm text-slate-600 mb-2">
                                                <span className="font-semibold">{apt.visit_type}</span>
                                            </div>
